...
import { z } from "zod";
import { CalendarIcon, ArrowRight, CheckCircle2, Wrench, Cog, Gauge, Droplet, ShieldCheck, Clock } from "lucide-react";
...
export const Route = createFileRoute("/book")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Schedule your Suzuki service appointment online. Choose a service, pick a date and time slot, and provide your vehicle details." },
      { property: "og:title", content: "Book a Service — Zeeshan Autos Workshop" },
      { property: "og:description", content: "Schedule your Suzuki workshop appointment in Hyderabad in under a minute." },
    ],
  }),
  component: BookPage,
});
...
const services = [
  { id: "oil", icon: Droplet, title: "Oil & Filter Service", time: "45 mins", price: "From PKR 6,500" },
  { id: "diag", icon: Gauge, title: "Computerized Diagnostics", time: "30 mins", price: "From PKR 2,500" },
  { id: "brake", icon: Cog, title: "Brake Inspection & Tuning", time: "60 mins", price: "From PKR 4,000" },
  ...
  { id: "annual", icon: Clock, title: "Annual Maintenance", time: "Half day", price: "From PKR 18,000" },
];
...
function BookPage() {
  ...
  const [pickupDrop, setPickupDrop] = useState<"yes" | "no">("no");
  const [model, setModel] = useState("");
  const [submitted, setSubmitted] = useState<null | { ref: string; service: string; when: string }>(null);
    ...
    return (
      <div className="px-6 lg:px-8 pt-20 pb-32 max-w-3xl mx-auto">
        <div className="rounded-3xl border border-primary/30 bg-gradient-to-br from-primary/15 via-background to-background p-10 lg:p-14 text-center">
          ...
          <p className="font-mono text-[10px] text-accent uppercase tracking-widest mb-3">Booking confirmed</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-border rounded-2xl overflow-hidden border border-border my-8">
            <div className="bg-background p-5">
              <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Reference</p>
              <p className="font-display text-lg">{submitted.ref}</p>
            </div>
            <div className="bg-background p-5">
              <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Service</p>
              <p className="font-display text-lg">{submitted.service}</p>
            </div>
            <div className="bg-background p-5">
              <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">When</p>
              <p className="font-display text-lg">{submitted.when}</p>
            </div>
          </div>
          <p className="text-sm text-muted-foreground max-w-md mx-auto mb-8">
            A service advisor will call you within 30 minutes (9 AM – 8 PM) to confirm your slot and answer any questions.
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            <button
              onClick={() => { setSubmitted(null); setServiceId(""); setDate(undefined); setTimeSlot(""); }}
            ...
            <Link to="/" className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground text-[11px] font-bold uppercase tracking-widest rounded-full hover:bg-primary-glow transition shadow-glow">
  ...
  return (
    <div className="px-6 lg:px-8 pt-16 pb-24 max-w-7xl mx-auto">
      <SectionHeading
        ...
        title={<>Book your <span className="text-primary">service</span></>}
      ...
      <form onSubmit={handleSubmit} className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-10">
          ...
          <section>
            <div className="flex items-baseline gap-3 mb-5">
              <span className="font-mono text-[10px] text-accent uppercase tracking-widest">Step 01</span>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className={cn("size-10 rounded-lg grid place-items-center", active ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary")}>
                        <s.icon className="size-4" />
                      </div>
                      <span className="font-mono text-[10px] text-muted-foreground uppercase tracking-widest">{s.time}</span>
                    </div>
                    <h3 className="font-display text-base uppercase tracking-tight mb-1">{s.title}</h3>
          ...
          <section>
            <div className="flex items-baseline gap-3 mb-5">
              <span className="font-mono text-[10px] text-accent uppercase tracking-widest">Step 02</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                ...
                <Select value={timeSlot} onValueChange={setTimeSlot}>
                  <SelectTrigger className="h-11"><SelectValue placeholder="Select a time" /></SelectTrigger>
                  <SelectContent>
          ...
          <section>
            <div className="flex items-baseline gap-3 mb-5">
              <span className="font-mono text-[10px] text-accent uppercase tracking-widest">Step 03</span>
              <h2 className="font-display text-2xl uppercase tracking-tight">Vehicle & contact</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="fullName">Full name</Label>
                <Input id="fullName" name="fullName" maxLength={80} required placeholder="Ahmed Khan" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="registration" name="registration" maxLength={20} required placeholder="AHB-1234" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <RadioGroup value={pickupDrop} onValueChange={(v) => setPickupDrop(v as "yes" | "no")} className="flex gap-6">
              ...
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="notes">Notes for the technician <span className="text-muted-foreground">(optional)</span></Label>
                <Textarea id="notes" name="notes" maxLength={500} rows={4} placeholder="Any specific concerns, sounds, or symptoms…" />
              </div>
            </div>
          </section>
        </div>
        ...
        <aside className="lg:sticky lg:top-24 h-fit">
          <div className="rounded-2xl border border-border bg-background p-6">
            <p className="font-mono text-[10px] text-accent uppercase tracking-widest mb-4">Booking summary</p>
            <dl className="space-y-4 text-sm">
              <div>
                <dt className="text-muted-foreground text-xs uppercase tracking-wider mb-1">Service</dt>
                <dd className="font-display">{services.find((s) => s.id === serviceId)?.title ?? "—"}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground text-xs uppercase tracking-wider mb-1">Date</dt>
                <dd className="font-display">{date ? format(date, "EEE, dd MMM yyyy") : "—"}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground text-xs uppercase tracking-wider mb-1">Time</dt>
                <dd className="font-display">{timeSlot || "—"}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground text-xs uppercase tracking-wider mb-1">Pickup & drop</dt>
                <dd className="font-display capitalize">{pickupDrop === "yes" ? "Yes — we'll collect" : "No — visit workshop"}</dd>
              </div>
            </dl>
            <button
              type="submit"
